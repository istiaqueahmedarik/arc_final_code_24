...
def gps_move(gpslang, gpslat):
    ...
    waypoints = []
    while currentLang == 0 and currentLang == 0:
        rospy.loginfo("Waiting for GPS data")
        rospy.sleep(1)
    waypoints.append(create_waypoint(currentLang, currentLat, 10))
    waypoints.append(create_waypoint(gpslang, gpslat, 10));
...
def stageControl():
    while(currentLang == 0 and currentLat == 0):
        rospy.loginfo("Waiting for GPS data")
        rospy.sleep(1)
    cur = 1
    while(1):
